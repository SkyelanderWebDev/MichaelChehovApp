...
import { RotateCcw, X, BookOpen, Sparkles } from "lucide-react";
import { TOOL_CATEGORIES } from "@/lib/toolData";
import FlybackModal from "./FlybackModal";
import POAJournal from "./POAJournal";
    ...
    >
      <Card className="w-full max-w-2xl min-h-64 sm:min-h-96 p-6 sm:p-12 rounded-2xl sm:rounded-3xl shadow-2xl relative animate-in zoom-in duration-500 bg-card border-4 border-accent overflow-y-auto max-h-[95vh]">
        <Button
          variant="ghost"
          size="icon"
          className="absolute top-3 right-3 sm:top-4 sm:right-4 hover:bg-accent/20 w-10 h-10"
          onClick={onClose}
        ...
        <div className="flex flex-col items-center justify-center space-y-4 sm:space-y-8 h-full">
          ...
          {hasScale ? (
            <div className="flex flex-col items-center space-y-4 sm:space-y-6">
              <div className="relative w-36 h-36 sm:w-56 sm:h-56 flex items-center justify-center">
                <div className="absolute inset-0 rounded-full border-4 sm:border-8 border-accent/30"></div>
                <div
                  className="absolute inset-0 rounded-full border-4 sm:border-8 border-accent border-t-transparent"
                  style={{
                    transform: `rotate(${(drawnTool.scaleValue! / 10) * 360}deg)`,
                    transition: 'transform 0.5s ease-out'
                  }}
                ></div>
                <span className="text-5xl sm:text-7xl font-bold font-serif text-primary" data-testid="text-scale-value">
                  {drawnTool.scaleValue}
                </span>
              </div>

              {showCategoryAsMain && (
                <h1 className="text-3xl sm:text-5xl font-bold font-serif text-center text-primary" data-testid="text-tool-name">
                  {drawnTool.categoryName}
                </h1>
              )}
              {showParentTool && (
              ...
              {showChildTool && (
                <p className="text-xl sm:text-2xl text-accent font-semibold italic mt-2">
            ...
            <>
              {showCategoryAsMain && (
                <h1 className="text-4xl sm:text-6xl font-bold font-serif text-center px-2 sm:px-4 text-primary" data-testid="text-tool-name">
                  {drawnTool.categoryName}
                </h1>
              )}
              {showParentTool && (
              ...
              {showChildTool && (
                <p className="text-2xl sm:text-3xl text-accent font-semibold italic mt-2 sm:mt-4">
                  "{drawnTool.childToolName}"
                </p>
              )}
            </>
            ...
            <div className="mt-4 sm:mt-6 flex flex-col items-center">
              ...
              <div className="flex items-center gap-3">
                <div className="relative w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center">
                  ...
                  <span className="text-2xl sm:text-3xl font-bold text-accent" data-testid="text-unveiled-value">
          ...
          {category?.description && (
            <p className="text-muted-foreground text-center max-w-md text-base sm:text-lg">
              {category.description}
            </p>
          )}
